...
- For HTTP requests, do not reuse a consumed `req.Body`. Instead:
  ...
  - Prefer configuring `req.GetBody` so the transport can recreate the body for redirects/retries
- To duplicate a stream while reading, use `io.TeeReader` or write to multiple sinks with `io.MultiWriter`
- Reusing buffered readers: call `(*bufio.Reader).Reset(r)` to attach to a new underlying reader
- For large payloads, avoid unbounded buffering; consider streaming, `io.LimitReader`, or on-disk temporary storage
- Use `io.Pipe` to stream without buffering the whole payload; always close the writer, use `CloseWithError(err)` on failures
- **Warning:** with `io.Pipe` (especially multipart), all writes must be sequential; out-of-order or parallel writes can corrupt the stream
...
- Use stdlib `testing` only; no third-party assert libraries
...
- Overusing unconstrained types (e.g., `any`); prefer specific types or generic type parameters with constraints
